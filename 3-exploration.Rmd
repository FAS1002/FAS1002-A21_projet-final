...
dfLatestWeek <- subset(dfVacWorld, date == latestdate )

dfCan <- subset(dfVacWorld, location == "Canada")

# Moyenne mondiale des vaccinations totales quotidiennes de la dernière semaine
DailyMeanW <- mean(dfLatest$daily_vaccinations)

# Moyenne canadienne des vaccinations totales quotidiennes de la dernière semaine
DailyMeanC <- 
...
## Portrait canadien épidémiologique de la prévention de la covid

```{r, include=FALSE}


plot5 <- ggplot(data = dfCan,
                aes(x = as.Date(date), y = people_fully_vaccinated, fill = Population))+
  geom_col()+
  scale_fill_viridis_d(option = "C")+
  theme_classic()+
  theme(axis.title.x = element_text(family = "Arial"))+
