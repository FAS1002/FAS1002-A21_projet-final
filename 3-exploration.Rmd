...
### Tableau comparatif des doses de vaccins par pays selon les dernières observations obtenues
```{r}
SommairePaysDernier <- dfPaysDosesDernier %>% 
  summarise(Moyenne_vaccins = mean(total_vaccinations, na.rm = TRUE),
            Médiane_vaccins = median(total_vaccinations, na.rm = TRUE),
            Min_vaccins = min(total_vaccinations, na.rm = TRUE),
            Max_vaccins = max(total_vaccinations, na.rm = TRUE),
            Ecart_interquartile = IQR(total_vaccinations, na.rm = TRUE),
            Déviation = sd(total_vaccinations, na.rm = TRUE),
            Variance = var(total_vaccinations, na.rm = TRUE),
            Données_manquantes = sum(is.na(dfPaysDosesDernier$total_vaccinations)))
...
Les graphiques ont été fait avec le @wickham_ggplot2_2016
